...
class DirectSum extends Operator {
	 ...
	 *
	 * @param Matrix $value
	 *        	The numeric value to concatenate/direct sum with the current base value
	 * @return $this The operation object, allowing multiple additions to be chained
	 * @throws Exception If the provided argument is not appropriate for the operation
	 */
	protected function directSumMatrix(Matrix $value) {
		...
		$this->matrix = array_merge ( $this->matrix, array_fill ( 0, $additionalRowCount, array_fill ( 0, $originalColumnCount, 0 ) ) );

		for($row = $originalRowCount; $row < $originalRowCount + $additionalRowCount; ++ $row) {
			array_splice ( $this->matrix [$row], $originalColumnCount, $additionalColumnCount, $value [$row - $originalRowCount] );
		}
