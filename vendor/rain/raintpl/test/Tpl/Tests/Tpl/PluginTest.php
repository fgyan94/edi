...
class PluginTest extends PHPUnit_Framework_TestCase {
	public function testDeclareHooks() {
		...
		$plugin->set_hooks ( array (
				'before_parse'
		) );
		$this->assertEquals ( array (
				'before_parse'
		), $plugin->declare_hooks () );
	}
	public function testSetOption() {
		$plugin = $this->getMock ( 'Rain\Tpl\Plugin', array (
				'set_param'
		) );
		$plugin->expects ( $this->once () )->method ( 'set_param' )->with ( $this->equalTo ( 'value' ) );
