...
class PluginContainerTest extends PHPUnit_Framework_TestCase {
	...
	public function testSetAndRun() {
		...
		$first = $this->getMock ( 'Rain\Tpl\IPlugin', array (
				'declare_hooks',
				'before',
				'custom',
				'set_options'
		) );
		$first->expects ( $this->once () )->method ( 'declare_hooks' )->will ( $this->returnValue ( array (
				'before',
				'after' => 'custom'
		) ) );
		$first->expects ( $this->once () )->method ( 'before' );
		$first->expects ( $this->once () )->method ( 'custom' );
		// second mock to test that all plugins are executed
		$second = $this->getMock ( 'Rain\Tpl\IPlugin', array (
				'declare_hooks',
				'before',
				'custom',
				'set_options'
		) );
		$second->expects ( $this->once () )->method ( 'declare_hooks' )->will ( $this->returnValue ( array (
				'before' => 'before'
		) ) );
		$second->expects ( $this->once () )->method ( 'before' );
		// set plugins and run them
		$this->container->set_plugin ( 'first', $first )->set_plugin ( 'second', $second )->run ( 'before', $context );
	...
	public function testRejectBadPlugin() {
		$plugin = $this->getMock ( 'Rain\Tpl\IPlugin', array (
				'declare_hooks',
				'set_options'
		) );
		$plugin->expects ( $this->once () )->method ( 'declare_hooks' )->will ( $this->returnValue ( array (
				'before' => 'no_method'
		) ) );
	...
	public function testRemovePlugin() {
		...
		$kept = $this->getMock ( 'Rain\Tpl\IPlugin', array (
				'declare_hooks',
				'remove',
				'set_options'
		) );
		$kept->expects ( $this->once () )->method ( 'declare_hooks' )->will ( $this->returnValue ( array (
				'remove'
		) ) );
		$kept->expects ( $this->once () )->method ( 'remove' );

		$removed = $this->getMock ( 'Rain\Tpl\IPlugin', array (
				'declare_hooks',
				'remove',
				'set_options'
		) );
		$removed->expects ( $this->once () )->method ( 'declare_hooks' )->will ( $this->returnValue ( array (
				'remove'
		) ) );
	...
	public function testCreateContext() {
		$context = $this->container->create_context ( array (
				'param' => 'value'
		) );
