...
class Xlsx extends BaseReader {
	 ...
	 *        	array of attributes (indexes are attribute name, values are value)
	...
	public function load($pFilename) {
		...
		// ~ http://schemas.openxmlformats.org/package/2006/relationships"
		$wbRels = simplexml_load_string ( $this->securityScanner->scan ( $this->getFromZipArchive ( $zip, 'xl/_rels/workbook.xml.rels' ) ), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions () );
		...
		foreach ( $rels->Relationship as $rel ) {
			switch ($rel ['Type']) {
				case 'http://schemas.openxmlformats.org/package/2006/relationships/metadata/core-properties' :
					$xmlCore = simplexml_load_string ( $this->securityScanner->scan ( $this->getFromZipArchive ( $zip, "{$rel['Target']}" ) ), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions () );
									...
									foreach ( $xmlSheet->autoFilter->filterColumn as $filterColumn ) {
										...
										if ($filterColumn->filters) {
											...
											foreach ( $filters->dateGroupItem as $dateGroupItem ) {
												...
												], ( string ) $dateGroupItem ['dateTimeGrouping'] )->setRuleType ( Column\Rule::AUTOFILTER_RULETYPE_DATEGROUP );
											}
										}
										...
										if ($filterColumn->customFilters) {
											...
											foreach ( $customFilters->customFilter as $filterRule ) {
												$column->createRule ()->setRule ( ( string ) $filterRule ['operator'], ( string ) $filterRule ['val'] )->setRuleType ( Column\Rule::AUTOFILTER_RULETYPE_CUSTOMFILTER );
											}
										}
										...
										if ($filterColumn->dynamicFilter) {
											...
											foreach ( $filterColumn->dynamicFilter as $filterRule ) {
												// Operator is undefined, but always treated as EQUAL
												$column->createRule ()->setRule ( null, ( string ) $filterRule ['val'], ( string ) $filterRule ['type'] )->setRuleType ( Column\Rule::AUTOFILTER_RULETYPE_DYNAMICFILTER );
										...
										if ($filterColumn->top10) {
											...
											foreach ( $filterColumn->top10 as $filterRule ) {
												$column->createRule ()->setRule ( (((isset ( $filterRule ['percent'] )) && ($filterRule ['percent'] == 1)) ? Column\Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_PERCENT : Column\Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_BY_VALUE), ( string ) $filterRule ['val'], (((isset ( $filterRule ['top'] )) && ($filterRule ['top'] == 1)) ? Column\Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_TOP : Column\Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_BOTTOM) )->setRuleType ( Column\Rule::AUTOFILTER_RULETYPE_TOPTENFILTER );
											}
										}
									}
								}
							...
							if ($zip->locateName ( dirname ( "$dir/$fileWorksheet" ) . '/_rels/' . basename ( $fileWorksheet ) . '.rels' )) {
								...
								if ($xmlSheet->drawing && ! $this->readDataOnly) {
									foreach ( $xmlSheet->drawing as $drawing ) {
										...
										$xmlDrawing = simplexml_load_string ( $this->securityScanner->scan ( $this->getFromZipArchive ( $zip, $fileDrawing ) ), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions () )->children ( 'http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing' );
