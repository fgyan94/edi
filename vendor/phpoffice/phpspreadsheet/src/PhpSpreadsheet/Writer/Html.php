...
class Html extends BaseWriter {
			...
			if ($cell instanceof Cell) {
				...
				if ($cell->getValue () instanceof RichText) {
					...
					if ($this->preCalculateFormulas) {
						$cellData = NumberFormat::toFormattedString ( $cell->getCalculatedValue (), $pSheet->getParent ()->getCellXfByIndex ( $cell->getXfIndex () )->getNumberFormat ()->getFormatCode (), [ 
								$this,
								'formatColor'
						] );
					} else {
						$cellData = NumberFormat::toFormattedString ( $cell->getValue (), $pSheet->getParent ()->getCellXfByIndex ( $cell->getXfIndex () )->getNumberFormat ()->getFormatCode (), [ 
								$this,
								'formatColor'
						] );
					}
	...
	private function calculateSpans() {
		...
		$sheetIndexes = $this->sheetIndex !== null ? [ 
				$this->sheetIndex
		] : range ( 0, $this->spreadsheet->getSheetCount () - 1 );
