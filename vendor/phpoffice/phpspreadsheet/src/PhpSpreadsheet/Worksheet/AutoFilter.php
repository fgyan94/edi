...
class AutoFilter {
	...
	private function dynamicFilterDateRange($dynamicRuleType, &$filterColumn) {
		...
		$ruleValues [] = [ 
				'operator' => AutoFilter\Column\Rule::AUTOFILTER_COLUMN_RULE_GREATERTHANOREQUAL,
				'value' => $val
		];
		$ruleValues [] = [ 
				'operator' => AutoFilter\Column\Rule::AUTOFILTER_COLUMN_RULE_LESSTHAN,
				'value' => $maxVal
		];
		...
		return [ 
				...
				'arguments' => [ 
						...
						'join' => AutoFilter\Column::AUTOFILTER_COLUMN_JOIN_AND
				]
		];
	}
					...
					foreach ( $rules as $rule ) {
						...
						if (($dynamicRuleType == AutoFilter\Column\Rule::AUTOFILTER_RULETYPE_DYNAMIC_ABOVEAVERAGE) || ($dynamicRuleType == AutoFilter\Column\Rule::AUTOFILTER_RULETYPE_DYNAMIC_BELOWAVERAGE)) {
							...
							$columnFilterTests [$columnID] = [ 
									...
									'arguments' => [ 
											'filterRules' => $ruleValues,
											'join' => AutoFilter\Column::AUTOFILTER_COLUMN_JOIN_OR
									]
							];
						} else {
							// Date based
