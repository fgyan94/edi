...
class Date {
	 ...
	 *
	 * @param float|int $excelTimestamp
	 *        	MS Excel serialized date/time value
	 * @param null|DateTimeZone|string $timeZone
	 *        	The timezone to assume for the Excel timestamp,
	...
	public static function dateTimeToExcel(DateTimeInterface $dateValue) {
		return self::formattedPHPToExcel ( $dateValue->format ( 'Y' ), $dateValue->format ( 'm' ), $dateValue->format ( 'd' ), $dateValue->format ( 'H' ), $dateValue->format ( 'i' ), $dateValue->format ( 's' ) );
	}
